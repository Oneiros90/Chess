...
public partial class Chessboard
{
    ...
    public Move[] Moves(Position piecePosition, bool allowAmbiguousCastle = false, bool generateSan = true)
    {
        ...
        {
            throw new PieceNotFoundException(this, piecePosition);
        }
    ...
    {
        for (var x = (short)Math.Max(0, piecePosition.X - 1); x <= Math.Min(7, piecePosition.X + 1); x++)
        {
            for (var y = (short)Math.Max(0, piecePosition.Y - 1); y <= Math.Min(7, piecePosition.Y + 1); y++)
            {
                if ((x != piecePosition.X || y != piecePosition.Y) &&
                    (board[x, y] == null || board[x, y].Color != board[piecePosition].Color))
                {
        ...
        if (piecePosition.Y % 7 == 0 && piecePosition.X == 4)
        {
            ...
            if (board[1, piecePosition.Y] is null && board[2, piecePosition.Y] is null &&
    ...
    {
        var step = (short)(board[piecePosition].Color == PieceColor.White ? 1 : -1);
        ...
        if (rightX < 8)
        {
            if (board[rightX, nextY] is not null && board[rightX, nextY].Color != board[piecePosition].Color)
            {
                positions.Add(new Position { X = rightX, Y = nextY });
                         ...
                         new Move(piecePosition, new Position { Y = nextY, X = rightX })
                             { Piece = board[piecePosition] }, board, step, 1))
            {
                positions.Add(new Position { X = rightX, Y = nextY });
            }
        }
        ...
        if (leftX > -1)
        {
            if (board[leftX, nextY] is not null && board[leftX, nextY].Color != board[piecePosition].Color)
            {
                positions.Add(new Position { X = leftX, Y = nextY });
                         ...
                         new Move(piecePosition, new Position { Y = nextY, X = leftX })
                             { Piece = board[piecePosition] }, board, step, -1))
            {
                positions.Add(new Position { X = leftX, Y = nextY });
            }
        ...
        // 2 forward
        if (((y == 1 && board[piecePosition].Color == PieceColor.White) ||
             (y == 6 && board[piecePosition].Color == PieceColor.Black))
    ...
    private static void GenerateBishopPositions(Position piecePosition, Chessboard board, List<Position> positions)
    {
        ...
        foreach (var direction in BishopDirections)
        {
            ...
            var currentPosition = (x: (short)(piecePosition.X + direction.x),
    ...
    private static void GenerateRookPositions(Position piecePosition, Chessboard board, ICollection<Position> positions)
    {
        foreach (var direction in RookDirections)
        {
            var x = (short)(piecePosition.X + direction.x);
            var y = (short)(piecePosition.Y + direction.y);
